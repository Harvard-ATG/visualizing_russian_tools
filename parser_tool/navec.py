...
def getSimilarLSH(text):
    ...
    for idx in indices:
        ...
        neighbors.append(
            {"word": vocabulary[idx], "vector": list(float(v) for v in navec[vocabulary[idx]]), "similarity": float(similarity)}
        )
    end = time.time()
...
def getSimilarBruteForce(text, tolerance=0.3):
    ...
    for word in vocabulary:
        ...
        if similarity > tolerance:
            above_tol.append({"word": word, "vector": list(float(v) for v in navec[word]), "similarity": float(similarity)})
    end_sim = time.time()
