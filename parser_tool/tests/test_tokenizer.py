...
class TestTokenizerHelpers(unittest.TestCase):
    ...
    def test_split_hyphenated(self):
        tests = [
            {"tokens": ["по-весеннему"], "expected": ["по-весеннему"]},
            {"tokens": ["француженкою-гувернанткой"], "expected": ["француженкою", "-", "гувернанткой"]},
            {"tokens": ["seventeen-year-old"], "expected": ["seventeen", "-", "year", "-", "old"]},
        ]
        for test in tests:
    ...
    def test_split_punctuation(self):
        tests = [
            {"tokens": ["фрукты=яблоко|вишня"], "expected": ["фрукты", "=", "яблоко", "|", "вишня"]},
            {"tokens": ["(13  мая  1876)"], "expected": ["(", "13  мая  1876", ")"]},
            {"tokens": ["(Students", " ", "and", "Faculty)."], "expected": ["(", "Students", " ", "and", "Faculty", ")."]},
        ]
        for test in tests:
            (tokens, expected) = test["tokens"], test["expected"]
    ...
    def test_canonical(self):
        tests = [{"token": "све́те", "expected": "свете"}, {"token": "отец´", "expected": "отец"}]
        for test in tests:
