(function ($) {
        ...
        var RadarChart = {
            draw: function (id, d, options) {
                ...
                var g = d3.select(id)
                    ...
                    .append("g")
                    .attr("transform", "translate(" + (cfg.TranslateX + 200) + "," + cfg.TranslateY + ")");
                ;
    ...
    async function make_chart(data, lemma_lookup) {
        ...
        get_radar_config().then(function (result) {
            ...
            //Legend titles
            var LegendOptions = ['Singular', 'Plural'];

            //Data
            var d = [
                radar_data.data[0].axes, radar_data.data[1].axes
            ]

            // get count totals
            var singular_total = 0;
            var plural_total = 0;

            for (var val in d[0]) {
                singular_total += d[0][val]["value"];
            }
            for (var val in d[1]) {
                plural_total += d[1][val]["value"];
            }
            $("#sg").text(singular_total);
            $("#pl").text(plural_total);
    ...
    $(document).ready(async function () {
        console.log("ready!");
        $("#output").hide();
        $.getJSON("/static/js/src/data/RNC.json", function (data) {
