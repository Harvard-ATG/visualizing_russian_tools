...
CSV_FIELD_NAMES = [
    ...
    "fshort_stressed",
    "plshort_stressed",
    "comparative_stressed",
    "extracomparatives_stressed",
    "stem",
    "ending",
    "mNOMsg",
    "mACCsgAN",
    ...
    for pair_id, pair_key in enumerate(sorted(pairs.keys()), start=1):
        ...
        inserts.append(
            [
                ...
                pair_index,
                "imperfective",
                lookup[imperfective]["lemma_id"],
                imperfective,
                lookup[imperfective]["lemma_count"],
            ]
        )
        inserts.append(
            [pair_id, pair_name, pair_index, "perfective", lookup[perfective]["lemma_id"], perfective, lookup[perfective]["lemma_count"]]
        )

    sql = (
        "INSERT INTO aspect_pair (pair_id, pair_name, pair_index, aspect, lemma_id, lemma_label, lemma_count) VALUES (?, ?, ?, ?, ?, ?, ?)"
    )
    for insert in inserts:
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", help="Name of the CSV file with russian spreadsheet data.", default="NewVisualizingRussian21January2019.csv"
    )
